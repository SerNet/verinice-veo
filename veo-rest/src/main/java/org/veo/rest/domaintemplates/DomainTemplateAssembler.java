...
/**
 * Takes domain template information and creates a {@link TransformDomainTemplateDto} from it.
 * Catalogs can be added by passing element DTOs from which catalog items will be created.
 */
...
class DomainTemplateAssembler {
  ...
  /**
   * Create a catalog from a set of element DTOs. The catalog is stored in the assembler and will be
   * added to the domain template when {@link #createDomainTemplateDto()} is called.
   */
  public void addCatalog(
      String catalogName,
      String elementPrefix,
      Map<String, AbstractElementDto> catalogElementById) {
    String catalogId = Key.newUuid().uuidValue();
    Map<String, TransformCatalogItemDto> catalogItemsById =
        createCatalogItems(catalogElementById, catalogId, elementPrefix);
    ...
    catalogItemsById.values().stream().forEach(i -> i.getElement().getLinks().clear());
  ...
  private Map<String, TransformCatalogItemDto> createCatalogItems(
    ...
    for (Map.Entry<String, AbstractElementDto> e : readElements.entrySet()) {
      ...
      itemDto.setId(Key.newUuid().uuidValue());
      elementDto.setOwner(
          SyntheticIdRef.from(itemDto.getId(), ElementOwner.class, CatalogItem.class));
  ...
  /**
   * Creates the opposite feature for the element defined by targetId. So for every link in an
   * element we create an externalTairoRef in the target of the link with the link data. Except for
   * it self.
   */
  private void createExternalTailoringReferences(
      String targetId, Map<String, TransformCatalogItemDto> catalogItems) {
    TransformCatalogItemDto targetItem = catalogItems.get(targetId);
    catalogItems.entrySet().stream()
        .filter(entries -> !entries.getKey().equals(targetId)) // exclude
        // self
        .map(Entry::getValue)
        .forEach(
            catalogItemDto -> {
              AbstractElementDto element = catalogItemDto.getElement();
              for (Entry<String, List<CustomLinkDto>> typedLinks : element.getLinks().entrySet()) {
                Stream<CustomLinkDto> allLinksToTarget =
                    typedLinks.getValue().stream()
                        .filter(link -> link.getTarget().getId().equals(targetId));
                allLinksToTarget.forEach(
                    link -> {
                      ...
                      targetItem.getTailoringReferences().add(referenceDto);
                    });
              }
            });
  }

  private void createTailoringReferences(
      AbstractElementDto value, Map<String, TransformCatalogItemDto> catalogItems) {
    ...
    value.getLinks().entrySet().stream()
        .forEach(
            e ->
                e.getValue()
                    .forEach(
                        l -> {
                          TransformCatalogItemDto itemDto = catalogItems.get(l.getTarget().getId());
                          TransformLinkTailoringReference referenceDto =
                              new TransformLinkTailoringReference();
                          referenceDto.setCatalogItem(

...
class DomainTemplateImportRestTest extends VeoRestTest {
    ...
    def "import domain template"() {
        ...
        post("/domaintemplates/", template, 201, UserType.CONTENT_CREATOR)

        and: "creating and fetching a new domain based on the template"
        post("/domaintemplates/${template.id}/createdomains", null, 204, UserType.ADMIN)
    ...
    private Map getTemplateBody() {
        def templateId = UUID.randomUUID().toString()
        return [
            'id': templateId,
            'name': "Import test template $templateId",
            'revision': 'latest',
            'templateVersion': '1.0',
            'abbreviation': 'ITT',
            'authority': 'JJ',
            'catalogs': [
                [
                    ...
                    'domainTemplate': [
                        'targetUri': "/domaintemplates/$templateId"],

...
class UpdateDomainUseCaseITSpec extends VeoSpringSpec {
    ...
    @Autowired
    private UpdateDomainUseCase useCase
    ...
    def "Migrate a scope referencing a risk definition"() {
        ...
        def dsgvoDomainV2 = runUseCase(dsgvoDomain.id, DSGVO_DOMAINTEMPLATE_V2_UUID)
    ...
    def "Migrate a scenario with probability"() {
        ...
        scenario.getPotentialProbability(dsgvoDomainV2).get(riskDefinitionRef).potentialProbability.idRef == 2
    }
    ...
    def "Migrate a unit with elements from a profile"() {
    ...
    Domain runUseCase(UUID domainId, UUID domainTemplateId) {
        executeInTransaction {
            useCase.execute(new UpdateDomainUseCase.InputData(domainId, domainTemplateId), NoRestrictionAccessRight.from(client.idAsString)).newDomain()
        }
    }
}
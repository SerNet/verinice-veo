...
public class DeleteUnitUseCaseSpec extends UseCaseSpec {
    ...
    def "Delete a unit with subunits" () {
        ...
        def subUnit = existingUnit.createSubUnit("Subunit")
        ...
        def input = new ChangeUnitUseCase.InputData(existingUnit, existingClient)
        def usecase = new DeleteUnitUseCase(clientRepository, unitRepository, transformContextProvider, repositoryProvider)
        usecase.execute(input)

        then: "the client for the unit is retrieved"
        1 * clientRepository.findClientsContainingUnit(_) >> [existingClient]
        ...
        1 * unitRepository.save({
            it.name == "Existing unit"
            it.state == Lifecycle.STORED_DELETED
        }, _, _) >> { it[0] }
    }
}

...
class DomainSensitiveElementValidator {
    ...
    element
        ...
        .forEach(
            link -> {
              ...
              var domain =
                  element.getOwningClient().get().getDomains().stream()
  ...
  private static void validateLinkTargetType(
      CustomLink link, Domain domain, LinkDefinition linkDefinition) {
    var targetType = link.getTarget().getModelType();
    if (!linkDefinition.getTargetType().equals(targetType)) {
      throw new IllegalArgumentException(
          String.format("Invalid target type '%s' for link type '%s'", targetType, link.getType()));
    }
  }
  ...
  private static void validateLinkTargetSubType(
    ...
    var targetSubType = link.getTarget().getSubType(domain).orElse(null);
    if (!linkDefinition.getTargetSubType().equals(targetSubType)) {
      throw new IllegalArgumentException(
          String.format(
              "Invalid target sub type '%s' for link type '%s'", targetSubType, link.getType()));
    }
  }
}

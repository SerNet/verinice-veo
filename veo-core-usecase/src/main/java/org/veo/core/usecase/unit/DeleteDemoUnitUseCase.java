...
/**
 * Delete the unit called 'Demo' from a client. This name is reserved for units
 * filled with example content. If the user has created a unit called "Demo" by
 * themselves (or if there is more than one "Demo"-unit present for whatever
 * reason) this function will refuse to work and log an error.
 */
...
public class DeleteDemoUnitUseCase
    ...
    public EmptyOutput execute(DeleteDemoUnitUseCase.InputData input) {
        var client = getClientUseCase.execute(new GetClientUseCase.InputData(input.getClientId()))
                                     .getClient();
        var demoUnits = getUnitsUseCase.execute(new GetUnitsUseCase.InputData(client,
                Optional.empty()))
                                       .getUnits()
                                       .stream()
                                       .filter(unit -> unit.getName()
        ...
        log.info("Deleting demo unit {}", demoUnits.get(0)
                                                   .getIdAsString());
        deleteUnitUseCase.execute(new DeleteUnitUseCase.InputData(demoUnits.get(0)
                                                                           .getId(),
                client));

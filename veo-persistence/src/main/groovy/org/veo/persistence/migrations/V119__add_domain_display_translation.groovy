...
class V119__add_domain_display_translation extends BaseJavaMigration {
    ...
    private static boolean migrateTable(String table, Sql context) {
        context.execute("""
            alter table $table
                add domain_display_translation jsonb;

            UPDATE $table
            SET domain_display_translation = json_build_object(
            ...
            alter table $table
                alter column domain_display_translation set not null;
        """.toString())
    }
}
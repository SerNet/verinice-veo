...
class V73__fix_tailoring_reference_type_check extends BaseJavaMigration {
    @Override
    void migrate(Context context) throws Exception {
        new Sql(context.connection).execute("""
        alter table profile_tailoring_reference
            drop constraint profile_tailoring_reference_referencetype_check;

        alter table profile_tailoring_reference
            add constraint profile_tailoring_reference_reference_type_range
                check ((reference_type >= 0) AND (reference_type <= 9));

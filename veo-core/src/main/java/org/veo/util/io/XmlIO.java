...
public final class XmlIO {
    ...
    public static <T> T read(String xsdSchemaFileName, String xmlFileName, Class<T> clazz)
            throws JAXBException, SAXException {
        // Schema und JAXBContext are thread safe
        SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
        Schema schema = (xsdSchemaFileName == null || xsdSchemaFileName.trim().length() == 0) ? null
                : schemaFactory.newSchema(new File(xsdSchemaFileName));
        JAXBContext jaxbContext = JAXBContext.newInstance(clazz.getPackage().getName());
        return read(jaxbContext, schema, xmlFileName, clazz);
    }

    public static <T> T read(JAXBContext jaxbContext, Schema schema, String xmlFileName,
    ...
    public static void write(String xsdSchemaFileName, String xmlFileName, Object jaxbElement)
            throws JAXBException, SAXException {
        SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
        Schema schema = (xsdSchemaFileName == null || xsdSchemaFileName.trim().length() == 0) ? null
                : schemaFactory.newSchema(new File(xsdSchemaFileName));
        JAXBContext jaxbContext = JAXBContext
                .newInstance(jaxbElement.getClass().getPackage().getName());
        write(jaxbContext, schema, xmlFileName, jaxbElement);
    }

    public static void write(JAXBContext jaxbContext, Schema schema, String xmlFileName,
